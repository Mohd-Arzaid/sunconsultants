import {
  Award,
  Battery,
  BookText,
  Building,
  CheckCircle,
  Cpu,
  FileCheck,
  FileWarning,
  Flame,
  GalleryHorizontal,
  Presentation,
  Radiation,
  Radio,
  Recycle,
  Scale,
  ShieldCheck,
  Video,
  Wifi,
  Zap,
} from "lucide-react";

// Navigation Data
export const NAVIGATION_DATA = {
  categories: [
    {
      id: 1,
      name: "BIS Mark (Foreign)",
      link: "/a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis",
      icon: CheckCircle,
      desktopOrder: 1,
      mobileOrder: 1,
    },
    
    {
      id: 2,
      name: "CRS Registration",
      link: "/what-is-crs-bis-or-crs-registration",
      icon: ShieldCheck,
      desktopOrder: 3,
      mobileOrder: 7,
    },
    {
      id: 3,
      name: "CE Certification",
      link: "/ce-certification",
      icon: ShieldCheck,
      desktopOrder: 4,
      mobileOrder: 14,
    },
    {
      id: 4,
      name: "ISI Mark (Indian)",
      link: "/a-guide-to-bis-certification-indian-bis",
      icon: Award,
      desktopOrder: 5,
      mobileOrder: 3,
    },
    {
      id: 5,
      name: "Scheme X",
      link: "/indian-bis-certification-under-scheme-x",
      icon: Cpu,
      desktopOrder: 6,
      mobileOrder: 8,
    },
    {
      id: 6,
      name: "EPR Registration",
      link: "/a-guide-on-how-to-obtain-epr-certificate",
      icon: Recycle,
      desktopOrder: 7,
      mobileOrder: 9,
    },
    {
      id: 7,
      name: "ROHS",
      link: "/restriction-of-hazardous-substance-rohs-certificate",
      icon: Radiation,
      desktopOrder: 8,
      mobileOrder: 15,
    },
    {
      id: 8,
      name: "BIS Certification",
      link: "/what-is-bis-certificate-indian-bis",
      icon: CheckCircle,
      desktopOrder: 9,
      mobileOrder: 4,
    },
    {
      id: 9,
      name: "Plastic Waste",
      link: "/epr-certificate-for-plastic-waste-management-pwm",
      icon: Recycle,
      desktopOrder: 10,
      mobileOrder: 10,
    },
    {
      id: 10,
      name: "EMI/EMC",
      link: "/emi-emc-certification",
      icon: Cpu,
      desktopOrder: 11,
      mobileOrder: 16,
    },
    {
      id: 11,
      name: "CDSCO Registration",
      link: "/cdsco-registration-certification",
      icon: FileWarning,
      desktopOrder: 12,
      mobileOrder: 5,
    },
    {
      id: 21,
      name: "APEDA Registration",
      link: "/apeda-registration-india",
      icon: FileCheck,
      desktopOrder: 22,
      mobileOrder: 21,
    },
    {
      id: 22,
      name: "ERDA Certificate",
      link: "/erda-certificate-india",
      icon: Zap,
      desktopOrder: 23,
      mobileOrder: 22,
    },
    {
      id: 23,
      name: "ICAT Certificate",
      link: "/icat-certificate-india",
      icon: ShieldCheck,
      desktopOrder: 24,
      mobileOrder: 23,
    },
    {
      id: 24,
      name: "SASO  Certification",
      link: "/saso-saber-certification",
      icon: Scale,
      desktopOrder: 25,
      mobileOrder: 24,
    },
    {
      id: 25,
      name: "STQC Certificate",
      link: "/stqc-certificate-india",
      icon: Cpu,
      desktopOrder: 26,
      mobileOrder: 25,
    },
    {
      id: 26,
      name: "TAC Certificate",
      link: "/tac-certificate-india",
      icon: CheckCircle,
      desktopOrder: 27,
      mobileOrder: 26,
    },

    {
      id: 27,
      name: "AIR",
      link: "/authorized-indian-representative",
      icon: Building,
      desktopOrder: 2,
      mobileOrder: 2,
    },

    




    {
      id: 12,
      name: "Battery Waste",
      link: "/epr-certificate-for-battery-waste-management-bwm",
      icon: Battery,
      desktopOrder: 13,
      mobileOrder: 11,
    },
    {
      id: 13,
      name: "TEC",
      link: "/information-about-tec-certificate-mtcte",
      icon: Radio,
      desktopOrder: 14,
      mobileOrder: 17,
    },
    {
      id: 14,
      name: "LMPC Registration",
      link: "/a-guide-on-how-to-obtain-lmpc-certificate",
      icon: BookText,
      desktopOrder: 15,
      mobileOrder: 6,
    },
    {
      id: 15,
      name: "PESO",
      link: "/information-about-peso-certification-peso-license-india",
      icon: Flame,
      desktopOrder: 16,
      mobileOrder: 12,
    },
    {
      id: 16,
      name: "BEE",
      link: "/bee-certification",
      icon: Zap,
      desktopOrder: 17,
      mobileOrder: 18,
    },
    {
      id: 17,
      name: "Legal Metrology",
      link: "/what-is-legal-metrology-or-lmpc-certificate",
      icon: Scale,
      desktopOrder: 18,
      mobileOrder: 7,
    },
    {
      id: 18,
      name: "WPC",
      link: "/information-about-wpc-certificate-eta-approval",
      icon: Wifi,
      desktopOrder: 19,
      mobileOrder: 13,
    },
    {
      id: 19,
      name: "CB Certification",
      link: "/cb-certification",
      icon: Award,
      desktopOrder: 20,
      mobileOrder: 19,
    },
    {
      id: 20,
      name: "NABL",
      link: "/nabl-certification-india",
      icon: Award,
      desktopOrder: 21,
      mobileOrder: 20,
    },
  
  ],
  updates: [
    {
      id: 1,
      name: "BIS QCO UPDATES",
      link: "/bis-qco-updates",
      icon: BookText,
    },
    {
      id: 2,
      name: "Ministry Updates",
      link: "/ministry-updates",
      icon: FileWarning,
    },
    {
      id: 3,
      name: "Upcoming Webinars",
      link: "/webinar",
      icon: Presentation,
    },
    {
      id: 4,
      name: "Video About BIS Certification",
      link: "/videos-about-bis-certification",
      icon: Video,
    },
    {
      id: 5,
      name: "Latest Blogs",
      link: "/Blogs",
      icon: BookText,
    },
    {
      id: 7,
      name: "All India First Licenses",
      link: "/all-india-first",
      icon: Award,
    },
    {
      id: 6,
      name: "Scheme X Products",
      link: "/SchemeX-products",
      icon: Building,
    },
    // {
    //   id: 7,
    //   name: "BIS Certification Products",
    //   link: "/bis-certification-products",
    //   icon: CheckCircle,
    // },
  ],
  gallery: [
    {
      id: 1,
      name: "International Audits",
      link: "/international-audits",
      icon: FileCheck,
    },
    {
      id: 2,
      name: "Seminars/Exhibitions",
      link: "/seminars-and-exhibitions",
      icon: GalleryHorizontal,
    },
  ],
  about: [
    {
      id: 1,
      name: "About Us",
      link: "/about",
      icon: Building,
    },
    {
      id: 2,
      name: "Message From Management",
      link: "/message-from-management-desk",
      icon: Presentation,
    },
    {
      id: 3,
      name: "Organization Chart",
      link: "/organization-chart",
      icon: Building,
    },
    {
      id: 4,
      name: "Expert BIS Panel",
      link: "/expert-bis-certification-panel-sun-certifications-india",
      icon: Award,
    },
    {
      id: 5,
      name: "Grant Letter",
      link: "/BIS-certification-grant-letter",
      icon: Award,
    },
    {
      id: 6,
      name: "Awards",
      link: "/awards-and-recognition",
      icon: Award,
    },
  ],
  faqs: [
    {
      id: 1,
      name: "BIS MARK (Foreign)",
      link: "/a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis#faqs",
      icon: CheckCircle,
    },
    {
      id: 2,
      name: "CRS Registration",
      link: "/what-is-crs-bis-or-crs-registration#faqs",
      icon: ShieldCheck,
    },
    {
      id: 3,
      name: "Scheme X",
      link: "/indian-bis-certification-under-scheme-x#faqs",
      icon: Cpu,
    },
    {
      id: 4,
      name: "ISI Mark (Indian)",
      link: "/a-guide-to-bis-certification-indian-bis#faqs",
      icon: Award,
    },
    {
      id: 5,
      name: "BIS Certification",
      link: "/what-is-bis-certificate-indian-bis#faqs",
      icon: CheckCircle,
    },
  ],
};
