/**
 * URL Mapping Configuration for Internationalized Routes
 * Based on Excel data provided for SEO-friendly URLs in different languages
 */

// URL mappings for each language
export const URL_MAPPINGS = {
  // English (base)
  en: {
    // Main pages
    about: "about",
    contact: "contact",
    sitemap: "sitemap",
    "privacy-policy": "privacy-policy",
    "terms-and-conditions": "terms-and-conditions",

    // Services - Fixed the key structure
    "cdsco-registration-certification": "cdsco-registration-certification",
    "indian-bis-certification-under-scheme-x":
      "indian-bis-certification-under-scheme-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis",
    "a-guide-on-how-to-obtain-epr-certificate":
      "a-guide-on-how-to-obtain-epr-certificate",
    "what-is-bis-certificate-indian-bis": "what-is-bis-certificate-indian-bis",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "a-guide-on-how-to-obtain-lmpc-certificate",
    "epr-certificate-for-plastic-waste-management-pwm":
      "epr-certificate-for-plastic-waste-management-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "what-is-legal-metrology-or-lmpc-certificate",
    "bis-isi-mark-certification":
      "bis-isi-mark-certification",
    "epr-certificate-for-battery-waste-management-bwm":
      "epr-certificate-for-battery-waste-management-bwm",
    "what-is-crs-bis-or-crs-registration":
      "what-is-crs-bis-or-crs-registration",
    "information-about-peso-certification-peso-license-india":
      "information-about-peso-certification-peso-license-india",
    "information-about-tec-certificate-mtcte":
      "information-about-tec-certificate-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "information-about-wpc-certificate-eta-approval",
    "restriction-of-hazardous-substance-rohs-certificate":
      "restriction-of-hazardous-substance-rohs-certificate",
    "bee-certification": "bee-certification",
    "ce-certification": "ce-certification",
    "emi-emc-certification": "emi-emc-certification",
    "cb-certification": "cb-certification",
    lhscable: "lhscable",
  },

  it: {
    // Main pages
    about: "chi-siamo",
    contact: "contatto",
    sitemap: "mappa-del-sito",
    "privacy-policy": "politica-sulla-privacy",
    "terms-and-conditions": "termini-e-condizioni",

    // Services
    "cdsco-registration-certification": "registrazione-certificazione-cdsco",
    "indian-bis-certification-under-scheme-x":
      "certificazione-bis-indiana-secondo-schema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "guida-alla-certificazione-bis-per-produttori-stranieri-bis-indiano",
    "a-guide-on-how-to-obtain-epr-certificate":
      "guida-ottenere-certificato-epr",
    "what-is-bis-certificate-indian-bis": "cose-il-certificato-bis-indiano",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "guida-ottenere-certificato-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "certificato-epr-gestione-rifiuti-plastici-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "che-cos-e-metrologia-legale-certificato-lmpc",
    "bis-isi-mark-certification":
      "guida-alla-certificazione-bis-bis-indiano",
    "epr-certificate-for-battery-waste-management-bwm":
      "certificato-epr-gestione-rifiuti-batterie-bwm",
    "what-is-crs-bis-or-crs-registration":
      "cose-il-crs-bis-o-registrazione-crs",
    "information-about-peso-certification-peso-license-india":
      "informazioni-certificazione-peso-licenza-peso-india",
    "information-about-tec-certificate-mtcte":
      "informazioni-certificato-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informazioni-certificato-wpc-approvazione-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "restrizione-sostanze-pericolose-certificato-rohs",
    "bee-certification": "certificazione-bee",
    "ce-certification": "certificazione-ce",
    "emi-emc-certification": "certificazione-emi-emc",
    "cb-certification": "certificazione-cb",
    lhscable: "cavo-lhs",
  },

  // French
  fr: {
    // Main pages
    about: "a-propos",
    contact: "contact",
    sitemap: "plan-du-site",
    "privacy-policy": "politique-de-confidentialite",
    "terms-and-conditions": "termes-et-conditions",

    // Services
    "cdsco-registration-certification": "enregistrement-certification-cdsco",
    "indian-bis-certification-under-scheme-x":
      "certification-bis-indienne-selon-schema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "guide-certification-bis-pour-fabricants-etrangers-bis-inde",
    "a-guide-on-how-to-obtain-epr-certificate": "guide-obtenir-certificat-epr",
    "what-is-bis-certificate-indian-bis":
      "quest-ce-que-le-certificat-bis-indien",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "guide-obtenir-certificat-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "certificat-epr-gestion-dechets-plastiques-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "qu-est-ce-que-metrologie-legale-certificat-lmpc",
    "bis-isi-mark-certification":
      "guide-certification-bis-bis-indien",
    "epr-certificate-for-battery-waste-management-bwm":
      "certificat-epr-gestion-dechets-batteries-bwm",
    "what-is-crs-bis-or-crs-registration":
      "quest-ce-que-le-crs-bis-ou-lenregistrement-crs",
    "information-about-peso-certification-peso-license-india":
      "informations-certification-peso-licence-peso-inde",
    "information-about-tec-certificate-mtcte":
      "informations-certificat-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informations-certificat-wpc-approbation-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "restriction-substances-dangereuses-certificat-rohs",
    "bee-certification": "certification-bee",
    "ce-certification": "certification-ce",
    "emi-emc-certification": "certification-emi-emc",
    "cb-certification": "certification-cb",
    lhscable: "cable-lhs",
  },

  // German
  de: {
    // Main pages
    about: "uber-uns",
    contact: "kontakt",
    sitemap: "sitemap",
    "privacy-policy": "datenschutzrichtlinie",
    "terms-and-conditions": "nutzungsbedingungen",

    // Services
    "cdsco-registration-certification": "cdsco-registrierung-zertifizierung",
    "indian-bis-certification-under-scheme-x":
      "indische-bis-zertifizierung-nach-schema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "leitfaden-zur-bis-zertifizierung-fuer-auslaendische-hersteller-indisches-bis",
    "a-guide-on-how-to-obtain-epr-certificate":
      "leitfaden-epr-zertifikat-erhalten",
    "what-is-bis-certificate-indian-bis":
      "was-ist-das-bis-zertifikat-indisches-bis",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "leitfaden-lmpc-zertifikat-erhalten",
    "epr-certificate-for-plastic-waste-management-pwm":
      "epr-zertifikat-kunststoffabfall-management-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "was-ist-gesetzliche-metrologie-lmpc-zertifikat",
    "bis-isi-mark-certification":
      "leitfaden-zur-bis-zertifizierung-indisches-bis",
    "epr-certificate-for-battery-waste-management-bwm":
      "epr-zertifikat-batterieabfall-management-bwm",
    "what-is-crs-bis-or-crs-registration":
      "was-ist-crs-bis-oder-crs-registrierung",
    "information-about-peso-certification-peso-license-india":
      "informationen-peso-zertifizierung-peso-lizenz-indien",
    "information-about-tec-certificate-mtcte":
      "informationen-tec-zertifikat-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informationen-wpc-zertifikat-eta-genehmigung",
    "restriction-of-hazardous-substance-rohs-certificate":
      "beschrankung-gefahrlicher-stoffe-rohs-zertifikat",
    "bee-certification": "bee-zertifizierung",
    "ce-certification": "ce-zertifizierung",
    "emi-emc-certification": "emi-emc-zertifizierung",
    "cb-certification": "cb-zertifizierung",
    lhscable: "lhs-kabel",
  },

  // Japanese
  ja: {
    // Main pages
    about: "watashitachi-ni-tsuite",
    contact: "otoiawase",
    sitemap: "saitomappu",
    "privacy-policy": "puraibashii-porishii",
    "terms-and-conditions": "riyou-kiyaku",

    // Services
    "cdsco-registration-certification": "cdsco-touroku-nintei",
    "indian-bis-certification-under-scheme-x": "indo-no-bis-nintei-sukimu-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "bis-nintei-gaikoku-seizousha-no-tame-no-gaido-india-no-bis",
    "a-guide-on-how-to-obtain-epr-certificate": "epr-shoumeisho-shutoku-gaido",
    "what-is-bis-certificate-indian-bis":
      "bis-shomeisho-toha-nani-ka-indo-no-bis",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "lmpc-shoumeisho-shutoku-gaido",
    "epr-certificate-for-plastic-waste-management-pwm":
      "purasuchikku-haikibutsu-kanri-epr-shoumeisho-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "houritsu-keiryougaku-lmpc-shoumeisho-to-wa",
    "bis-isi-mark-certification": "bis-nintei-gaido-india-no-bis",
    "epr-certificate-for-battery-waste-management-bwm":
      "denchi-haikibutsu-kanri-epr-shoumeisho-bwm",
    "what-is-crs-bis-or-crs-registration": "crs-bis-toha-nani-ka-crs-toroku",
    "information-about-peso-certification-peso-license-india":
      "peso-nintei-peso-raisensu-indo-ni-tsuite-no-jouhou",
    "information-about-tec-certificate-mtcte":
      "tec-shoumeisho-mtcte-ni-tsuite-no-jouhou",
    "information-about-wpc-certificate-eta-approval":
      "wpc-shoumeisho-eta-shounin-ni-tsuite-no-jouhou",
    "restriction-of-hazardous-substance-rohs-certificate":
      "yuugai-busshitsu-seigen-rohs-shoumeisho",
    "bee-certification": "bee-nintei",
    "ce-certification": "ce-nintei",
    "emi-emc-certification": "emi-emc-nintei",
    "cb-certification": "cb-nintei",
    lhscable: "lhs-keeburu",
  },

  // Korean
  ko: {
    // Main pages
    about: "hoesa-sogae",
    contact: "yeonrak",
    sitemap: "saiteu-maep",
    "privacy-policy": "gaein-jeongbo-cheolibanghim",
    "terms-and-conditions": "iyong-yakgwan",

    // Services
    "cdsco-registration-certification": "cdsco-deungnok-injeung",
    "indian-bis-certification-under-scheme-x":
      "indo-bis-injeung-scheme-x-haenghaeng",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "indo-bis-waeoe-jejo-eopeul-wihan-bis-injeung-gaideu",
    "a-guide-on-how-to-obtain-epr-certificate":
      "epr-injeungseo-chwideuk-gaideub",
    "what-is-bis-certificate-indian-bis":
      "bis-jeungmyeongseo-ga-mueos-indo-bis",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "lmpc-injeungseo-chwideuk-gaideub",
    "epr-certificate-for-plastic-waste-management-pwm":
      "peullaeseutik-pyegimul-gwanli-epr-injeungseo-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "beopjeok-gyeryanghak-lmpc-injeungseo-ran",
    "bis-isi-mark-certification": "bis-injeung-gaideu-indo-bis",
    "epr-certificate-for-battery-waste-management-bwm":
      "baeteri-pyegimul-gwanli-epr-injeungseo-bwm",
    "what-is-crs-bis-or-crs-registration": "crs-bis-i-mueos-inga-crs-deunglog",
    "information-about-peso-certification-peso-license-india":
      "peso-injeung-peso-raisensu-indo-e-daehan-jeongbo",
    "information-about-tec-certificate-mtcte":
      "tec-injeungseo-mtcte-e-daehan-jeongbo",
    "information-about-wpc-certificate-eta-approval":
      "wpc-injeungseo-eta-seungin-e-daehan-jeongbo",
    "restriction-of-hazardous-substance-rohs-certificate":
      "yuhae-mulji-jeyak-rohs-injeungseo",
    "bee-certification": "bee-injeung",
    "ce-certification": "ce-injeung",
    "emi-emc-certification": "emi-emc-injeung",
    "cb-certification": "cb-injeung",
    lhscable: "lhs-keibeul",
  },

  // Indonesian
  id: {
    // Main pages
    about: "tentang-kami",
    contact: "hubungi-kami",
    sitemap: "peta-situs",
    "privacy-policy": "kebijakan-privasi",
    "terms-and-conditions": "syarat-dan-ketentuan",

    // Services
    "cdsco-registration-certification": "registrasi-sertifikasi-cdsco",
    "indian-bis-certification-under-scheme-x":
      "sertifikasi-bis-india-di-bawah-skema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "panduan-sertifikasi-bis-untuk-produsen-asing-bis-india",
    "a-guide-on-how-to-obtain-epr-certificate":
      "panduan-cara-mendapatkan-sertifikat-epr",
    "what-is-bis-certificate-indian-bis": "apa-itu-sertifikat-bis-bis-india",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "panduan-cara-mendapatkan-sertifikat-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "sertifikat-epr-untuk-pengelolaan-limbah-plastik-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "apa-itu-metrologi-hukum-atau-sertifikat-lmpc",
    "bis-isi-mark-certification":
      "panduan-sertifikasi-bis-bis-india",
    "epr-certificate-for-battery-waste-management-bwm":
      "sertifikat-epr-untuk-pengelolaan-limbah-baterai-bwm",
    "what-is-crs-bis-or-crs-registration":
      "apa-itu-crs-bis-atau-registrasi-crs",
    "information-about-peso-certification-peso-license-india":
      "informasi-tentang-sertifikasi-peso-lisensi-peso-india",
    "information-about-tec-certificate-mtcte":
      "informasi-tentang-sertifikat-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informasi-tentang-sertifikat-wpc-persetujuan-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "pembatasan-zat-berbahaya-sertifikat-rohs",
    "bee-certification": "sertifikasi-bee",
    "ce-certification": "sertifikasi-ce",
    "emi-emc-certification": "sertifikasi-emi-emc",
    "cb-certification": "sertifikasi-cb",
    lhscable: "kabel-lhs",
  },

  // Thai
  th: {
    // Main pages
    about: "kieokap-rao",
    contact: "tidtor-rao",
    sitemap: "phaen-phang-websai",
    "privacy-policy": "nayobai-khwam-pen-suan-tua",
    "terms-and-conditions": "kho-kamhad-kan-chai-borikan",

    // Services
    "cdsco-registration-certification": "kan-long-thabien-kar-rap-rong-cdsco",
    "indian-bis-certification-under-scheme-x":
      "bis-prathiap-india-taem-dai-tae-skema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "khumanam-kanraprong-bis-samrab-puuphlit-thangchat-bis-india",
    "a-guide-on-how-to-obtain-epr-certificate":
      "khumue-witha-dai-rap-bai-rap-rong-epr",
    "what-is-bis-certificate-indian-bis":
      "bis-certificate-khue-a-rai-bis-india",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "khumue-witha-dai-rap-bai-rap-rong-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "bai-rap-rong-epr-samrap-kan-borihan-khaya-plastic-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "metrology-kotmai-rue-bai-rap-rong-lmpc-khue-arai",
    "bis-isi-mark-certification":
      "khumanam-kanraprong-bis-bis-india",
    "epr-certificate-for-battery-waste-management-bwm":
      "bai-rap-rong-epr-samrap-kan-borihan-khaya-battery-bwm",
    "what-is-crs-bis-or-crs-registration":
      "crs-bis-khue-a-rai-rab-phit-thab-crs",
    "information-about-peso-certification-peso-license-india":
      "khomul-kieokap-kar-rap-rong-peso-bai-anuyat-peso-india",
    "information-about-tec-certificate-mtcte":
      "khomul-kieokap-bai-rap-rong-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "khomul-kieokap-bai-rap-rong-wpc-kan-anumati-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "kan-jam-kad-sathu-antarai-bai-rap-rong-rohs",
    "bee-certification": "kar-rap-rong-bee",
    "ce-certification": "kar-rap-rong-ce",
    "emi-emc-certification": "kar-rap-rong-emi-emc",
    "cb-certification": "kar-rap-rong-cb",
    lhscable: "sai-lhs",
  },

  // Vietnamese
  vi: {
    // Main pages
    about: "ve-chung-toi",
    contact: "lien-he",
    sitemap: "so-do-trang-web",
    "privacy-policy": "chinh-sach-bao-mat",
    "terms-and-conditions": "dieu-khoan-va-dieu-kien",

    // Updates
    "bis-qco-updates": "cap-nhat-bis-qco",
    "ministry-updates": "cap-nhat-bo",
    "international-audits": "kiem-toan-quoc-te",
    "seminars-and-exhibitions": "hoi-thao-va-trien-lam",
    webinar: "webinar",
    "videos-about-bis-certification": "video-ve-chung-nhan-bis",

    // Services
    "cdsco-registration-certification": "dang-ky-chung-nhan-cdsco",
    "indian-bis-certification-under-scheme-x":
      "chung-nhan-bis-an-do-theo-scheme-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "huong-dan-chung-nhan-bis-cho-nha-san-xuat-nuoc-ngoai-bis-an-do",
    "a-guide-on-how-to-obtain-epr-certificate":
      "huong-dan-cach-lay-chung-chi-epr",
    "what-is-bis-certificate-indian-bis": "chung-chi-bis-la-gi-bis-an-do",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "huong-dan-cach-lay-chung-chi-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "chung-chi-epr-cho-quan-ly-rac-thai-nhua-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "do-luong-phap-ly-hoac-chung-chi-lmpc-la-gi",
    "bis-isi-mark-certification":
      "huong-dan-chung-nhan-bis-bis-an-do",
    "epr-certificate-for-battery-waste-management-bwm":
      "chung-chi-epr-cho-quan-ly-rac-thai-pin-bwm",
    "what-is-crs-bis-or-crs-registration": "crs-bis-la-gi-hoac-dang-ky-crs",
    "information-about-peso-certification-peso-license-india":
      "thong-tin-ve-chung-nhan-peso-giay-phep-peso-an-do",
    "information-about-tec-certificate-mtcte":
      "thong-tin-ve-chung-chi-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "thong-tin-ve-chung-chi-wpc-phe-duyet-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "han-che-chat-doc-hai-chung-chi-rohs",
    "bee-certification": "chung-nhan-bee",
    "ce-certification": "chung-nhan-ce",
    "emi-emc-certification": "chung-nhan-emi-emc",
    "cb-certification": "chung-nhan-cb",
    lhscable: "cap-lhs",
  },

  // Spanish
  es: {
    // Main pages
    about: "acerca-de-nosotros",
    contact: "contacto",
    sitemap: "mapa-del-sitio",
    "privacy-policy": "politica-de-privacidad",
    "terms-and-conditions": "terminos-y-condiciones",

    // Services
    "cdsco-registration-certification": "registro-certificacion-cdsco",
    "indian-bis-certification-under-scheme-x":
      "certificacion-bis-india-bajo-esquema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "guia-certificacion-bis-para-fabricantes-extranjeros-bis-indio",
    "a-guide-on-how-to-obtain-epr-certificate":
      "guia-como-obtener-certificado-epr",
    "what-is-bis-certificate-indian-bis": "que-es-el-certificado-bis-bis-indio",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "guia-como-obtener-certificado-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "certificado-epr-gestion-residuos-plasticos-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "que-es-metrologia-legal-certificado-lmpc",
    "bis-isi-mark-certification":
      "guia-certificacion-bis-bis-indio",
    "epr-certificate-for-battery-waste-management-bwm":
      "certificado-epr-gestion-residuos-baterias-bwm",
    "what-is-crs-bis-or-crs-registration": "que-es-crs-bis-o-registro-crs",
    "information-about-peso-certification-peso-license-india":
      "informacion-certificacion-peso-licencia-peso-india",
    "information-about-tec-certificate-mtcte":
      "informacion-certificado-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informacion-certificado-wpc-aprobacion-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "restriccion-sustancias-peligrosas-certificado-rohs",
    "bee-certification": "certificacion-bee",
    "ce-certification": "certificacion-ce",
    "emi-emc-certification": "certificacion-emi-emc",
    "cb-certification": "certificacion-cb",
    lhscable: "cable-lhs",
  },

  // Dutch
  nl: {
    // Main pages
    about: "over-ons",
    contact: "contact",
    sitemap: "sitemap",
    "privacy-policy": "privacybeleid",
    "terms-and-conditions": "algemene-voorwaarden",

    // Services
    "cdsco-registration-certification": "cdsco-registratie-certificering",
    "indian-bis-certification-under-scheme-x":
      "indiaas-bis-certificaat-volgens-schema-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "gids-voor-bis-certificering-voor-buitenlandse-fabrikanten-indiaas-bis",
    "a-guide-on-how-to-obtain-epr-certificate":
      "gids-hoe-epr-certificaat-verkrijgen",
    "what-is-bis-certificate-indian-bis":
      "wat-is-het-bis-certificaat-indiaas-bis",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "gids-hoe-lmpc-certificaat-verkrijgen",
    "epr-certificate-for-plastic-waste-management-pwm":
      "epr-certificaat-plastic-afvalbeheer-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "wat-is-wettelijke-metrologie-lmpc-certificaat",
    "bis-isi-mark-certification":
      "gids-voor-bis-certificering-indiaas-bis",
    "epr-certificate-for-battery-waste-management-bwm":
      "epr-certificaat-batterij-afvalbeheer-bwm",
    "what-is-crs-bis-or-crs-registration": "wat-is-crs-bis-of-crs-registratie",
    "information-about-peso-certification-peso-license-india":
      "informatie-peso-certificering-peso-licentie-india",
    "information-about-tec-certificate-mtcte":
      "informatie-tec-certificaat-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "informatie-wpc-certificaat-eta-goedkeuring",
    "restriction-of-hazardous-substance-rohs-certificate":
      "beperking-gevaarlijke-stoffen-rohs-certificaat",
    "bee-certification": "bee-certificering",
    "ce-certification": "ce-certificering",
    "emi-emc-certification": "emi-emc-certificering",
    "cb-certification": "cb-certificering",
    lhscable: "lhs-kabel",
  },

  // Arabic
  ar: {
    // Main pages
    about: "hawlana",
    contact: "aitasal",
    sitemap: "kharitat-almawqae",
    "privacy-policy": "siyasat-alkhasusiat",
    "terms-and-conditions": "al-shurut-wal-ahkam",

    // Updates
    "bis-qco-updates": "tahdeethat-bis-qco",
    "ministry-updates": "tahdeethat-alwizarat",
    "international-audits": "altadqeeq-alduwali",
    "seminars-and-exhibitions": "alnadawat-wal-maarid",
    webinar: "webinar",
    "videos-about-bis-certification": "video-hawl-shahdat-bis",

    // Services
    "cdsco-registration-certification": "tasjeel-shahdat-cdsco",
    "indian-bis-certification-under-scheme-x":
      "shahadat-bis-alhind-tahata-almukhatat-x",
    "a-guide-to-bis-certification-for-foreign-manufacturers-indian-bis":
      "dalil-shahadat-bis-lilmusanein-alajnabiyin-bis-alhind",
    "a-guide-on-how-to-obtain-epr-certificate":
      "daleel-kayfiat-alhusul-ala-shahdat-epr",
    "what-is-bis-certificate-indian-bis": "ma-huwa-shahadat-bis-bis-alhind",
    "a-guide-on-how-to-obtain-lmpc-certificate":
      "daleel-kayfiat-alhusul-ala-shahdat-lmpc",
    "epr-certificate-for-plastic-waste-management-pwm":
      "shahdat-epr-lidarat-almukhalafat-alblastikiat-pwm",
    "what-is-legal-metrology-or-lmpc-certificate":
      "ma-hi-almetrologia-alqanuniat-aw-shahdat-lmpc",
    "bis-isi-mark-certification": "dalil-shahadat-bis-bis-alhind",
    "epr-certificate-for-battery-waste-management-bwm":
      "shahdat-epr-lidarat-mukhalafat-albatariyat-bwm",
    "what-is-crs-bis-or-crs-registration": "ma-huwa-crs-bis-aw-tasjeel-crs",
    "information-about-peso-certification-peso-license-india":
      "maelumat-hawl-shahdat-peso-rukhsat-peso-alhind",
    "information-about-tec-certificate-mtcte":
      "maelumat-hawl-shahdat-tec-mtcte",
    "information-about-wpc-certificate-eta-approval":
      "maelumat-hawl-shahdat-wpc-muwafaqat-eta",
    "restriction-of-hazardous-substance-rohs-certificate":
      "quyud-almawad-alkhatirat-shahdat-rohs",
    "bee-certification": "shahdat-bee",
    "ce-certification": "shahdat-ce",
    "emi-emc-certification": "shahdat-emi-emc",
    "cb-certification": "shahdat-cb",
    lhscable: "kabl-lhs",
  },
};

// Reverse mapping for finding English URL from localized URL
export const REVERSE_URL_MAPPINGS = {};

// Create reverse mappings
Object.keys(URL_MAPPINGS).forEach((lang) => {
  REVERSE_URL_MAPPINGS[lang] = {};
  Object.keys(URL_MAPPINGS[lang]).forEach((englishUrl) => {
    const localizedUrl = URL_MAPPINGS[lang][englishUrl];
    REVERSE_URL_MAPPINGS[lang][localizedUrl] = englishUrl;
  });
});

/**
 * Get localized URL for a given English URL and language
 * @param {string} englishUrl - The English URL
 * @param {string} lang - The target language
 * @returns {string} The localized URL
 */
export const getLocalizedUrl = (englishUrl, lang) => {
  if (lang === "en" || !URL_MAPPINGS[lang]) {
    return englishUrl;
  }
  return URL_MAPPINGS[lang][englishUrl] || englishUrl;
};

/**
 * Get English URL from localized URL
 * @param {string} localizedUrl - The localized URL
 * @param {string} lang - The current language
 * @returns {string} The English URL
 */
export const getEnglishUrl = (localizedUrl, lang) => {
  if (lang === "en" || !REVERSE_URL_MAPPINGS[lang]) {
    return localizedUrl;
  }
  return REVERSE_URL_MAPPINGS[lang][localizedUrl] || localizedUrl;
};

/**
 * Generate full localized path with language prefix
 * @param {string} englishUrl - The English URL
 * @param {string} lang - The target language
 * @returns {string} The full localized path
 */
export const getFullLocalizedPath = (englishUrl, lang) => {
  if (lang === "en") {
    return `/${englishUrl}`;
  }
  const localizedUrl = getLocalizedUrl(englishUrl, lang);
  return `/${lang}/${localizedUrl}`;
};

/**
 * Get all available languages for a URL
 * @param {string} englishUrl - The English URL
 * @returns {Object} Object with language codes as keys and localized URLs as values
 */
export const getAllLocalizedUrls = (englishUrl) => {
  const result = {};
  Object.keys(URL_MAPPINGS).forEach((lang) => {
    result[lang] = getFullLocalizedPath(englishUrl, lang);
  });
  return result;
};

/**
 * Validate if a localized URL is valid for a given language
 * @param {string} localizedUrl - The localized URL to validate
 * @param {string} lang - The language code
 * @returns {boolean} True if the URL is valid for the language
 */
export const isValidLocalizedUrl = (localizedUrl, lang) => {
  if (lang === "en") {
    return true; // English URLs are always valid
  }

  if (!URL_MAPPINGS[lang]) {
    return false;
  }

  // Check if the localized URL exists in the reverse mapping
  return REVERSE_URL_MAPPINGS[lang] && REVERSE_URL_MAPPINGS[lang][localizedUrl];
};

/**
 * Get supported languages for a specific URL
 * @param {string} englishUrl - The English URL
 * @returns {Array} Array of supported language codes for this URL
 */
export const getSupportedLanguagesForUrl = (englishUrl) => {
  const supportedLanguages = ["en"]; // English is always supported

  Object.keys(URL_MAPPINGS).forEach((lang) => {
    if (URL_MAPPINGS[lang][englishUrl]) {
      supportedLanguages.push(lang);
    }
  });

  return supportedLanguages;
};
