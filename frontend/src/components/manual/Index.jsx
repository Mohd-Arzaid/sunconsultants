...
const Index = () => {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 md:px-12 h-full max-w-[88rem] mx-auto">
        {/* Mobile Menu Button */}
        <button
        ...
        {/* Mobile Navigation */}
        <div
          className={`md:hidden fixed inset-0 bg-white/70 backdrop-blur-lg transition-transform duration-300 ${
          ...
          <div className="flex flex-col items-start pt-20 px-4">
            {SECTIONS.map((item) => (
              ...
              >
                <div
                  className={`text-base font-semibold font-geist tracking-wider uppercase transition-colors duration-300 ${
                    item === activeSection ? "text-blue-900" : "text-blue-950"
                  }`}
                >
                  {item}
                </div>
                {item === activeSection && (
                  <div className="w-full h-0.5 bg-blue-900 mt-2" />
                )}
              </div>
            ))}
          </div>
